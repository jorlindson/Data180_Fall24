...
author: 
  name: Josh Orlin
  email: "orlinj@dickinson.edu"
date: '`r format(Sys.Date(), 9,17,24")`'
output: 
  html_document
---
...
```{r}
my_df[my_df$Preference=="Twitter",]
```
...
my_func<- function(my_vec) {
  if(value>5){return("This element is greater than 5!")}
  if(value<=5){ return("This element is NOT greater than 5!")}
}
```
...
get_fibonacci = function(n) {
 if (n==1 | n==2) {1}
  else {get_fibonacci(n-1)+ get_fibonacci(n-2)}}
